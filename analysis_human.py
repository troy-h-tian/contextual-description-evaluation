...
OUTPUT_CSV  = "results_human_v2.csv"

clip_model = preprocess = sbert = nli = None

def setup():
    global clip_model, preprocess, sbert, nli
...
def nli_entailment(context_text, caption):
    ...
    # handle both list and single dict return formats
...
def main():
    ...
    # BERTScore — batch compute all at once
    ...
    plot_df = merged.dropna(subset=[
        "clipscore","context_sim","nli_entail","bertscore_f",
        "sighted_overall","blv_overall"
    ])
    print(f"Rows with full data: {len(plot_df)}")

    metrics  = ["clipscore", "context_sim", "nli_entail", "bertscore_f"]
    xlabels  = ["CLIPScore", "Context Sim (SBERT)", "NLI Entailment", "BERTScore F1"]
    ...
    fig, axes = plt.subplots(len(ratings), len(metrics), figsize=(18, 16))
    ...
    plt.tight_layout()
    plt.savefig("human_ratings_vs_metrics_v2.png", dpi=150, bbox_inches="tight")
    print("Saved plot to human_ratings_vs_metrics_v2.png")
